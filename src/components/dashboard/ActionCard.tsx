...
export const ActionCard = ({ icon: Icon, title, description, onClick }: ActionCardProps) => {
  ...
  return (
    <Card 
      className="h-full hover-lift-premium transition-all duration-300 bg-action-card border border-brand-purple/10 backdrop-blur-sm cursor-pointer group"
      onClick={handleClick}
    >
      <CardContent className="p-6 flex flex-col items-center text-center space-y-4">
        <div className="p-3 rounded-lg bg-brand-purple/10 group-hover:bg-brand-purple/20 transition-colors">
          <Icon className="h-6 w-6 text-brand-purple hover-scale" />
        </div>
